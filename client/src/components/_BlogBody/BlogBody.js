          ...
          props.blogData.map(blog => {
            ...
            return (
              <div className="col-md-4 float-right d-flex" key={blog.blogId}>
                  ...
                  <Link to={`/articles/${blog.blogId}`}><img
                  src={blog.cover}
                ...
                <div className="text p-4  d-block">
                  <div className="topper d-flex align-items-center">
                      <div className="py-2 pl-3 pr-3 align-self-stretch">{time}</div>
                  </div>
                  <Link className='text-dark' style = {{'text-decoration': 'none'}} to={`/articles/${blog.blogId}`}><h3 className="heading mb-3 text-capitalize">
                  {blog.title}
                  </h3></Link>
                  <p className='text-secondary'>{blog.body}</p>
                  <p>
                      <Link className='btn-custom' to ={`/articles/${blog.blogId}`}>
